...
class StandLightAgent:
    def __init__(self, user, task):
        ...
        self.T_reset = 3
        self.q_table = defaultdict(lambda: [0.1, 0.0, 0.0]) # Still/OFF/ON

    ''' Function to train the agent '''
    def learn(self, state, action, reward, next_state):
        self.time_step = max(self.time_step, 1)
        curr_state = [state[2], state[4], state[5], state[8]]
        next_state = [next_state[2], next_state[4], next_state[5], next_state[8]]
    ...
    def get_action(self, state):
        ...
        curr_state = [state[2], state[4], state[5], state[8]] # Brightness, Ceiling, Stand, TV
